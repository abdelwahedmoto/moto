...
const Fournisseur = () => {  
  ...
  const {createFournisseur,fournissuers,ModifierFournissuer,SupprimerFournisseur} = UseMainContext();
   ...
   const AjouterFournissuer = (event) => {
    ...
    createFournisseur(nom, telephone, adress);
    //createProduit()
  };
  //Modifer
  const handleFournisseur = (e) => {
    ...
    ModifierFournissuer(id, nom, telephone, adress);
  };
  // delete produit
  ...
  const handleUpdate = (id) => {
     const prod = fournissuers.filter((state) => state.id === id);
     setNom(prod[0].nom);
  ...
  return (
    ...
    <form action="" className=" w-[50%]">
      <div className=" flex flex-col gap-1 mt-3">
        <label className=" text-slate-900/70 font-medium ">Nom Fournissuer</label>
        <input
          value={nom}
          className=" border border-slate-300 px-3 py-2 outline-none rounded-lg focus:border-blue-400"
          type="text"
          placeholder="entrer nom Fournissuer"
      ...
      <tbody className="">
        {fournissuers?.map((item) => (
          <tr key={item.id} className="hover:bg-blue-300/10 text-center">
            ...
            <td className="pt-5 pl-5 pr-5 pb-5 ">{item.nom_f}</td>
